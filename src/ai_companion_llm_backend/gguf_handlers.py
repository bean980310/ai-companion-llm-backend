...
class GGUFCausalModelHandler(BaseCausalModelHandler):
    ...
    def load_model(self):
        if self.enable_langchain:
            ...
            self.model = Llama(model_path=self.local_model_path, lora_path=self.local_lora_model_path, n_gpu_layers=self.n_gpu_layers, split_mode=llama_cpp.LLAMA_SPLIT_MODE_NONE, n_ctx=2048)
    ...
    def generate_answer(self, history, **kwargs):
        if self.enable_langchain:
            ...
            response = self.model.create_chat_completion(messages=prompt, seed=self.seed, temperature=self.temperature, top_k=self.top_k, top_p=self.top_p, repeat_penalty=self.repetition_penalty, max_tokens=self.max_tokens, stream=True)
