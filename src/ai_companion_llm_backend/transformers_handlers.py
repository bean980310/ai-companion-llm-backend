...
class TransformersUnifiedModelHandler(BaseModelHandler):
    ...
    def load_model(self):
        ...
        if self.enable_langchain:
            if self.processor is not None:
                self.langchain_integrator = LangchainIntegrator(provider=("self-provided", "transformers"), model=self.model, processor=self.processor, max_tokens=self.max_new_tokens, temperature=self.temperature, top_k=self.top_k, top_p=self.top_p, repetition_penalty=self.repetition_penalty, verbose=True)
            else:
                self.langchain_integrator = LangchainIntegrator(provider=("self-provided", "transformers"), model=self.model, tokenizer=self.tokenizer, max_tokens=self.max_new_tokens, temperature=self.temperature, top_k=self.top_k, top_p=self.top_p, repetition_penalty=self.repetition_penalty, verbose=True)
    ...
    def generate_answer(self, history, **kwargs):
        ...
        generated_text = decoder.decode(outputs[0][input_len:], skip_special_tokens=True)
    ...
    def get_settings(self):
        return GenerationConfig(max_new_tokens=self.max_new_tokens, do_sample=True, temperature=self.temperature, top_k=self.top_k, top_p=self.top_p, repetition_penalty=self.repetition_penalty)

    def load_template(self, messages):
        applier = self.processor if self.processor is not None else self.tokenizer
        kwargs = dict(add_generation_prompt=True, tokenize=False, enable_thinking=self.enable_thinking)
        if self.use_tools:
...
class TransformersCausalModelHandler(BaseCausalModelHandler):
    ...
    def load_template(self, messages):
        if self.use_tools:
            return self.tokenizer.apply_chat_template(messages, tools=self.tools, add_generation_prompt=True, return_tensors="pt", tokenize=False, enable_thinking=self.enable_thinking)
        else:
            return self.tokenizer.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt", tokenize=False, enable_thinking=self.enable_thinking)
...
class TransformersVisionModelHandler(BaseVisionModelHandler):
    ...
    def generate_answer(self, history, **kwargs):
        try:
            ...
            _ = self.model.generate(**inputs, generation_config=self.config, streamer=streamer)
    ...
    def load_template(self, messages):
        if self.image_input:
            if self.use_tools:
                return self.processor.apply_chat_template(messages, tools=self.tools, add_generation_prompt=True, return_tensors="pt", tokenize=False, enable_thinking=self.enable_thinking)
            else:
                return self.processor.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt", tokenize=False, enable_thinking=self.enable_thinking)
        else:
            if self.use_tools:
                return self.tokenizer.apply_chat_template(messages, tools=self.tools, add_generation_prompt=True, return_tensors="pt", tokenize=False, enable_thinking=self.enable_thinking)
            else:
